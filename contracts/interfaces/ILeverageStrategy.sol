// SPDX-License-Identifier: GPL-3.0-or-later
/*
________                                          ______________________ 
__  ___/___  ____________________________ ___________  __ \__    |_  __ \
_____ \_  / / /__  __ \_  ___/  _ \_  __ `__ \  _ \_  / / /_  /| |  / / /
____/ // /_/ /__  /_/ /  /   /  __/  / / / / /  __/  /_/ /_  ___ / /_/ / 
/____/ \__,_/ _  .___//_/    \___//_/ /_/ /_/\___//_____/ /_/  |_\____/  
              /_/                                                        
*/

pragma solidity ^0.8.0;

interface ILeverageStrategy {
    function getTotalwstETHDeposited() external view returns (uint256);
    function withdrawFromVault(uint256 wstethAmoumt) external ;
    function depositFromVault(uint wstEthAmount) external;
}