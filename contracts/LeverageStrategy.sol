...
contract LeverageStrategy is AccessControl {
    ...
    function setTokenIndex(
        uint _TokenIndex
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        TokenIndex = _TokenIndex;
    }

    function setBPTAddress(
        address _bptAddress
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function _depositAndCreateLoan(
        uint256 _wstETHAmount,
        uint256 _debtAmount,
        uint256 _N
    ) internal {
        ...
        require(
            IERC20(wsteth).approve(address(crvUSDController), _wstETHAmount),
            "Approval failed"
        );
    ...
    function _joinPool(
        ...
        require(
            IERC20(usdc).approve(address(balancerVault), usdcAmount),
            "Approval failed"
        );
        ...
        uint256 joinKind = uint256(
            IBalancerVault.JoinKind.TOKEN_IN_FOR_EXACT_BPT_OUT
        );
    ...
    function _withdrawInvestment(
        ...
        bytes calldata extraStrategyData
    ) external {
        // Exit Aura position
    ...
    function _exchangeCRVUSDtoUSDC(uint256 _dx) internal {
        require(
            crvUSD.approve(address(crvUSDUSDCPool), _dx),
            "Approval failed"
        );
    ...
    function _depositAllAura() internal {
        require(
            d2dusdcBPT.approve(
                address(auraBooster),
                d2dusdcBPT.balanceOf(address(this))
            ),
            "Approval failed"
        );
    ...
    function _depositAura(uint ammount) internal {
        require(
            d2dusdcBPT.approve(address(auraBooster), ammount),
            "Approval failed"
        );
